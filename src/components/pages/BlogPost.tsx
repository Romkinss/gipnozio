    ...
    if (slug) {
      ...
      api.articles.get(slug).then(async data => {
        if (data) {
            ...
            if (redirect) {
                window.location.href = '/' + redirect.to_path, { replace: true };
            } else {
                setLoading(false);
            }
        }
  ...
  if (!article) return (
      <PublicLayout>
          ...
          <div className="min-h-[50vh] flex flex-col items-center justify-center text-center px-4">
            ...
            <a href="/blog" className="text-gold hover:underline">Вернуться в блог</a>
          </div>
      </PublicLayout>
  );
                 ...
                 <aside className="hidden lg:block relative">
                     <div className="sticky top-24 space-y-8">
                         ...
                         <div className="bg-dark text-white p-6 rounded-xl text-center relative overflow-hidden">
                             ...
                             <a href="/login" className="block w-full py-2 bg-gold text-dark font-bold rounded hover:bg-white transition-colors text-sm">
                                 Попробовать
                             </a>
                         </div>
                     </div>
                ...
                <div className="mt-16 pt-12 border-t border-gray-100">
                    ...
                    <div className="grid md:grid-cols-3 gap-6">
                        {relatedArticles.map(rel => (
                            <a key={rel.id} href={`/blog/${rel.slug}`} className="group block bg-gray-50 rounded-xl overflow-hidden hover:shadow-lg hover:shadow-gold/10 transition-all duration-300">
